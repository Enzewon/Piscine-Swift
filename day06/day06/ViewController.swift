...
class ViewController: UIViewController {
    ...
    var shapes: [Shape] = []
    var colors: [UIColor] = [.red, .blue, .brown, .black]
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let theTapGesture = UITapGestureRecognizer(target: self, action: #selector(touchGesture(_:)))
...
extension ViewController: MoveControllerDelegate {
    func touchShapeBegan(withTouches touches: Set<UITouch>, andEvent event: UIEvent?, andShape shape: Shape) {
        view.bringSubview(toFront: shape)
    }
}
